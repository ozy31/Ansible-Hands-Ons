...
```yml
    - name: Create db user with name 'remoteUser' and password 'XXXXX1234' with all database privileges
      mysql_user:
        name: remoteUser
        password: "XXXXX1234"
        login_user: "root"
        login_password: "XXXXX1234"
        priv: '*.*:ALL,GRANT'
        state: present
...
```yml
    ...
    - name: Replace a default entry with our own
      lineinfile:
        ...
        regexp: '172\.20\.1\.101'
        line: "$link = mysqli_connect('{{ hostvars['db_server'].ansible_host }}', 'remoteUser', 'XXXXX1234', 'ecomdb');"
